...
from models import db, Session, InteractionLog, Decision, Debrief, KnowledgeCheck
...
def register_routes(app):
    ...
    def start_session():
        ...
        return jsonify({
            ...
            "scenario": {
                ...
                "diagram":              engine.scenario.get("diagram"),
            },
    ...
    @app.post("/api/sessions/<session_id>/knowledge-check")
    def submit_knowledge_check(session_id):
        session = Session.query.get_or_404(session_id)
        data    = request.get_json(force=True)
        timing  = data.get("timing")

        if timing not in ("pre", "post"):
            abort(400, "timing must be 'pre' or 'post'")
        ...
        mc_score = {
            item_id: responses.get(item_id) == correct
            for item_id, correct in answer_key.items()
        }
        mc_total = sum(mc_score.values())

        existing = KnowledgeCheck.query.filter_by(
            session_id=session_id, timing=timing
        ).first()

        if existing:
            existing.responses_json = responses
            existing.mc_score_json  = mc_score
            existing.mc_total       = mc_total
        else:
            kc = KnowledgeCheck(
                session_id     = session_id,
                timing         = timing,
                responses_json = responses,
                mc_score_json  = mc_score,
                mc_total       = mc_total,
            )
            db.session.add(kc)

        db.session.commit()
        return jsonify({"mc_total": mc_total, "mc_score": mc_score}), 201

    @app.get("/api/sessions/<session_id>/knowledge-check/<timing>")
    def get_knowledge_check(session_id, timing):
        kc = KnowledgeCheck.query.filter_by(
            session_id=session_id, timing=timing
        ).first_or_404()
        return jsonify({
            "timing":    kc.timing,
            "responses": kc.responses_json,
            "mc_score":  kc.mc_score_json,
            "mc_total":  kc.mc_total,
        })
    ...
    def debug_session(session_id):
        ...
        kc_pre  = KnowledgeCheck.query.filter_by(session_id=session_id, timing="pre").first()
        kc_post = KnowledgeCheck.query.filter_by(session_id=session_id, timing="post").first()
        ...
        return jsonify({
            ...
            "knowledge_checks": {
                "pre":  {"mc_total": kc_pre.mc_total,  "mc_score": kc_pre.mc_score_json}  if kc_pre  else None,
                "post": {"mc_total": kc_post.mc_total, "mc_score": kc_post.mc_score_json} if kc_post else None,
            },
        })
